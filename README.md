...
| Assignment sound | **one soundboard per composition**, played **once** the moment you declare yours (real flow *and* rehearsal) — and **on the click only**: the addon plays **no** sound by itself (the intermission start file is manual, `/gideon sound test start`), on the Master channel; `/gideon sound on\|off` mutes it, `/gideon sound test 1v3r\|2v2r\|3v1r` plays one on request | `Core/Sound.lua` (pure table) + four Ogg files in `Sound/` |
| Which boss may open the panel | a **delivered default target** (encounter id `3445` = *Entombed Sentinels*, all difficulties, no command needed) plus a **persisted allow-list of encounter ids**: `/gideon boss <id>`, `/gideon boss list` (with provenance), `/gideon boss clear` (**explicitly** drops the delivered default = nothing opens), optional name list, `/gideon idlog on\|off` to read the real id in game; `/gideon inter on` = manual override for the next encounter; `/gideon diag` = read-only health report | pure decision in `Core/BossFilter.lua` + the delivered constants in `Core/Config.lua` + the pure report in `Core/Diag.lua` |
...
The policy is **configurable** (`/gideon ping`, persisted in
...
1. before the pull, `/gideon` → **PLACE INTERMISSION PANEL** (or `/gideon inter place`):
   ...
   *Options > Keybindings*, then the **OK** button under the illustration (or **`/gideon inter ok`** from the keyboard) to save the position (the panel
...
2. before the pull **you have nothing to configure**: the addon already targets
   *Entombed Sentinels* (encounter id `3445`, every difficulty — see §3.4). To
   target **another** boss, `/gideon boss <id>` (the *encounter id*, read in game with
   `/gideon idlog on`). If someone **cleared** the target on purpose (`/gideon boss
...
To test the whole flow **now**, on any boss, `/gideon inter on` is the **manual
...
```bash
/gideon lock           # freeze the panels where they are (persisted)
/gideon unlock         # let them be dragged again (also the UNLOCK PANEL button)
/gideon resetposition  # bring the main panel, the intermission panel and the ping help window back to the center
```
...
```bash
/gideon sim inter   # "Intermission group": the panel opens RIGHT AWAY, click your composition,
                # correct it with REDO, then close it YOURSELF (X or Close). One single cycle.
/gideon sim style   # STYLE SHOWCASE: the three cards, the whole typography, the palette with its
                # hex codes, THE guild card, the animations (no boss, no raid)
/gideon sim style 1 # same, with the guild card (the only style) applied to the live preview
/gideon sim anim off# switch the showcase animations off (persisted; "on" puts them back)
/gideon style 1     # the card style of the intermission panel, persisted (the ONLY style there is;
                # "shipped", "bare", "nu" and "encartnu" all name it too)
/gideon style       # which card style a fight really uses, and that there is no choice left
/gideon sim ping    # PING HELP (= /gideon pinghelp): how to bind one key per ping, and how to ping yourself
/gideon sim stop    # close the rehearsal or the help window (= the Close button, = the cross)
```
...
click on one of the three buttons, in the real flow **as in the `/gideon sim inter`
...
automatically** — not at the auto-open, not in `/gideon sim inter`, not when the panel
...
```bash
/gideon sound test start       # hear the (now manual-only) intermission start sound
```

```bash
/gideon sound                  # is the sound enabled? (and how to change it)
/gideon sound on | off         # enable/disable it (persisted; an unknown value is refused)
/gideon sound test 1v3r        # hear one soundboard now, without waiting for a fight
/gideon sound test 2v2r        # (also: 3v1r)
/gideon sound test start       # hear the intermission start sound now
```
...
4. in game, `/reload`, then `/gideon sound test 1v3r` (then `2v2r`, `3v1r`): the chat
   names the file it played — that is how you confirm **which** file you heard.
   The sound is muted if the preference is off (`/gideon sound off` then
   `/gideon sound test` says so): `/gideon sound on` first.
...
### 3.4 Which boss may open the panel (`/gideon boss`) — **the target is delivered**
...
```bash
/gideon boss            # what will open at the next pull (effective target + source + idlog + override)
/gideon boss 3445       # ADD an encounter id (persisted; replaying it changes nothing)
/gideon boss name <text> # add the SECONDARY criterion: the exact encounter NAME
/gideon boss list       # the two lists WITH their provenance, the override, the encounters seen
/gideon boss clear      # empty both lists AND drop the delivered default (nothing opens)
/gideon idlog on | off  # log every encounter seen (id / name / difficulty / group)
/gideon diag            # health report: the 4 sound files + the target + the idlog + the ping
```
...
| --- | --- | --- |
| encounter id (**primary** criterion) | **`3445`** | `ENCOUNTER_START` arg1, read in game by the raid lead on **2026-09-24** (heroic pull, 20 players, `/gideon idlog on`): `encounter seen: id=3445 name=Sentinelles inhumées difficulty=15 group=20` |
...
- **`/gideon boss clear`** (an *explicit* choice) → the delivered default is
  **dropped too**: nothing opens by itself any more, until `/gideon boss <id>` names a
  target again. The two states are stored differently (`bossTargetCleared`) and
  reported differently (`/gideon boss`, `/gideon boss list`, `/gideon diag`), so a deliberate
  clear can never be mistaken for a lost configuration, and the delivered default
  never comes back behind the player's back;
- a player's own ids are **added to** the delivered default (`/gideon boss 2594` →
  both `2594` and `3445` open, `/gideon boss list` labels each entry *added by you* or
...
- `/gideon boss <id>` refuses anything that is not a **positive integer** (`abc`,
  `0`, `-3`, `12.5`, `1e3`) **without persisting anything** — same mechanics as
  `/gideon lang`, `/gideon ping` and `/gideon sound`;
- `/gideon inter on` is the **manual override**: it arms the panel for the **next**
...
4. note the `id=` value (**`/gideon boss list`** shows the last 10 encounters seen, so
   you can read it back after the fight);
5. `/gideon boss <id>` once — done: that boss opens the panel too. `/gideon idlog off`
   when you are finished measuring.

#### `/gideon diag` — the health report, in one command
...
- **sound on and audible** → **nothing is played, ever**, the four lines read
  *NOT TESTED*, and the report gives the two ways to get the verdict: set the
  master volume to 0 (`/console Sound_MasterVolume 0`), run `/gideon diag`, put it
  back (`/console Sound_MasterVolume 1`); or hear a file **on purpose** with
  `/gideon sound test 1v3r|2v2r|3v1r|start` (those *do* play a sound);
...
listed in `GideonRaid.toc`** is **never** loaded. `/gideon diag` writes nothing, sends
...
generator. `/gideon style <name>` accepts `1` / `shipped` / `bare` / `nu` / `encartnu`,
...
During `/gideon inter place` (and the **PLACE INTERMISSION PANEL** button) the panel
...
composition** on that panel: drag it where you want, validate with the **OK** button (or `/gideon inter ok` from the keyboard)
...
#### The style showcase (`/gideon sim style`) — seeing the design in game
...
`/gideon sim style` therefore opens a **scrollable style showcase** (no boss, no raid;
...
- **the animations**: a fade-in at the opening and a discreet pulse on the first
  card, switched with `/gideon sim anim on|off` (persisted).
...
- In game: `/gideon lang` shows the detected language, the effective one and the
  preference; `/gideon lang auto`, `/gideon lang en` and `/gideon lang fr` rule on it and
...
├── GideonRaid.toc
├── GideonRaid.lua     <- wiring: ADDON_LOADED, PLAYER_LOGIN, ENCOUNTER_*, /gideon, /gideon lang
...
│   │                     `bossTargetCleared` marker (explicit `/gideon boss clear`)
│   ├── Pairing.lua
│   ├── Intermission.lua
...
│   │                     + the `/gideon diag` probe: reads 2 sound CVars, calls
...
│                                    0.13.0 (`/gideon sound test start`), never played
...
> **Command name.** Since **0.13.5** the base command is **`/gideon`** (long form
> `/gideonraid`). The former `/gr` is **deliberately not registered any more**: in
> game `/gr` is a common shortcut for talking to the group, and an addon must never
> take a chat shortcut away from the player — the raid lead hit exactly that ("I use
> it to talk to my group"). Every example below uses `/gideon …`; entries of the
> CHANGELOG older than 0.13.5 still spell it `/gr …` because that is what shipped
> then. Sub-commands and behaviour are unchanged.
...
target (measured id 3445) + `/gideon diag`**, the **three pictures + one word panel with
...
`/gideon boss clear` vs player addition, and the **removal** of the old placement text
...
(`/gideon inter ok` also works)**, **the explicit 44/64 px font of the word and no Blizzard font
...
never configured, pure decision, good/wrong/unreadable id, empty name, `/gideon boss`
and `/gideon idlog` wiring, idlog ring, manual override),
...
and present on disk, bounded `/gideon sound` preference, one playback per assignment,
...
`pingpolicy_spec.lua` (20 — ping roles and policies), `diag_spec.lua` (19 — `/gideon diag`:
