--[[--------------------------------------------------------------------------
    ...
    tout le blabla"). It is read during a fight, so it carries NO sentence at all:
      - BEFORE a click: the THREE IMAGE BUTTONS, stacked VERTICALLY, each one
        drawing the raid lead's own screenshot of the orbs (Texture/*.tga, a
        256 px box, generated by tools/make_textures.py). Nothing else - no
        title, no state line, no role line, no action line, no key reminder. The
        order is FROZEN by Core/Layout.INTERMISSION_CHOICE_ORDER (3V1R, 2V2R,
      ...
      - AFTER a click: ONE WORD and nothing else - "Ping" (1V3R) and "Chasseur"
        ...
        window - plus CORRECT, which brings the three pictures back. The colors
        come from Core/Layout.THEME (one constant, never a color written here).
...
local function ensurePanel()
    ...
    -- FontStrings, because a FontString is created with its font: "wordBig"
    -- (GameFontNormalHuge) is the middle composition - the biggest element of the
    -- WINDOW, as the raid lead asked - and "word" (GameFontNormalLarge) carries
    -- the two others. Core/Layout.wordBlockId() decides which one the layout
    -- contains; the color comes from Core/Layout.THEME.
    p.word = p:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    p.word:SetJustifyH("CENTER")
    p.word:SetText("")
    p.word:Hide()

    p.wordBig = p:CreateFontString(nil, "OVERLAY", "GameFontNormalHuge")
    p.wordBig:SetJustifyH("CENTER")
    p.wordBig:SetText("")
    p.wordBig:Hide()

    p.buttons = {}
    -- THE THREE IMAGE BUTTONS: the player clicks the PICTURE of the composition
    -- they see above their head. The order (top to bottom) is FROZEN by Core
    ...
    -- this layer writes no label at all, and the number/colour wording of the
    -- former buttons is gone.
    ...
    -- the ONLY button of the panel after a click, and the raid lead confirmed it.
...
function UI.IntermissionRefresh()
    ...
    if setupMode then
        -- PLACEMENT MODE (before the pull): the three PICTURES, exactly as during
        -- the fight, plus OK to validate. NO TEXT AT ALL - the former placement
        -- blob (drag here / bind a key / ready in N s / the plan) is gone: the
        -- procedure lives in docs/ and in the chat commands, not over the raid.
        spec.showChoices = true
        spec.showOk = true
        return UI.ApplyLayout(p, Layout.intermissionPanel(spec), panelElements(p))
    end
