...
| 1c | Language layer: strings, resolution, `/gideon lang` | busted + lua5.1 | on every commit | CI + local |
...
| 1f | Soundboards: state → file table, one playback per assignment, one playback per intermission for the START sound, `/gideon sound` preference (pure) | busted + lua5.1 | on every commit | CI + local |
| 1g | Which boss may OPEN the panel: pure allow-list of encounter ids (`/gideon boss`), the DELIVERED default target (id 3445 + the two names), explicit clear vs never configured, name criterion, idlog ring, manual override (pure) | busted + lua5.1 | on every commit | CI + local |
| 1h | `/gideon diag` health report: sound file list, SILENCE GATE of the audio probe, verdict per file, report lines (pure) | busted + lua5.1 | on every commit | CI + local |
...
the DELIVERED target: never configured vs explicit `/gideon boss clear` vs player
addition), `load_spec.lua` (49),
`bossfilter_spec.lua` (45 — which boss may open the panel: the delivered default
target, pure decision + wiring of `/gideon boss` and `/gideon idlog`), `sound_spec.lua`
...
(20), `diag_spec.lua` (19 — `/gideon diag`: silence gate, verdict per sound file,
...
| **Panel preferences** | `/gideon lock` / `/gideon unlock` persisted (`lockPanel`), **`lockPanel` false by default** (the main panel is draggable), a **total lock resolver** (only a real boolean locks, anything else unlocks) and a **total position resolver** (only a known `Config.POSITION_POINTS` anchor survives; a hand-edited `point` falls back to `CENTER`), and the **one-time migration** that unlocks a legacy `lockPanel = true` once (no schema marker) before respecting the player's own choice |
| Command `/gideon ping` | `ping` prints the policy and its meaning, `ping color|none` persists it and changes the panel (the action line and the ping to use), unknown value refused **without writing anything**, value reloaded after a `/reload` simulation |
...
3. the wiring end to end (with the API stub): English by default, `GetLocale()`
   ...
   raising, `/gideon lang` printing the detected + effective + preferred language and
   the rule, `/gideon lang fr|en|auto` persisted in `GideonRaidDB.locale` and applied
...
- the **ping help view**: the four pings to bind (`Ping, Warning, On My Way,
  ...
  nothing);
- the **whole `/gideon sim` argument** parsed by `parseCommand`: `inter` with its aliases
  (`group`, `groupe`), `ping`, `stop`, and the **refusal of any trailing option**
  (`/gideon sim inter cycles=3`), of an unknown sub-command and of an invalid argument —
...
6. the slash handler (`/gideon show`, `/gideon status`, `/gideon plan`, `/gideon inter status`,
...
7. `ENCOUNTER_START` (with its instance arguments) **arms the schedule without
   ...
   chat explains the safe default (`/gideon boss <id>`, `/gideon inter on`). The four
...
13. `ENCOUNTER_END` closes the panel, disarms the schedule and no opening happens
    afterwards;
14. the **placement mode** (button of the main panel or `/gideon inter place`) shows
...
16. disabling (`/gideon inter off`) is honoured, placement included;
...
21. the ping help window (`/gideon sim ping` **and** `/gideon pinghelp`) carries the binding
...
25. **the assignment soundboard** (Step 1f, `sound_spec.lua`): the sound of the
    **declared** composition is played **once**, on the `Master` channel, with the
    ...
    a new intermission and a new rehearsal re-arm it, and `/gideon sound off` silences
...
28. **the AUTO-OPEN BOSS FILTER** (Step 1g, `bossfilter_spec.lua` +
    ...
    **delivered** id `3445` and on the two delivered names (the English official one
    ...
    delivered one and `/gideon boss list` labels each entry (*addon default* / *added by
    you*); `/gideon boss 3445` is idempotent; `/gideon boss clear` is an **explicit** state
    ...
    hand-edited `bossTargetCleared = "oui"` does **not** drop it; `/gideon boss` refuses
    a value that is not a positive integer **without persisting anything**; a boss
    that is not the target is refused (`noMatch`); `/gideon idlog on` prints the
    `encounter seen: id=…` line and memorizes it (10 max); `/gideon inter on` arms the
    ...
    with no Lua error;
29. **the intermission START sound** (Step 1f): `/gideon sound test start` names and
    ...
    rehearsal plays it once per cycle, and `/gideon sound off` silences it too;
30. **the `/gideon diag` health report** (Step 1h, `diag_spec.lua`): the four sound files
    ...
    `/gideon diag` calls `PlaySoundFile` **zero** times when the client is audible or
...
2. `/reload` → `/gideon` → check: "Your partner: <Name>" + the list of pairs.
3. `/gideon status` → "assignment OK, 2 pairs".
4. `/gideon reset` → then `/reload` → the panel shows "No GIDEON assignment."
   without error.

### 3.2 Out of combat, inside the instance — reading in a raid

5. On a raid boss **before the pull**: `/gideon` must display exactly the same pairs
...
8. `/gideon` typed 20 times: no duplicated frame, no slowdown.
9. Move the panel (drag), `/reload`, check the position is kept; then `/gideon lock`
   (the panel must refuse to move and say so), `/gideon unlock` (it moves again) and
   `/gideon resetposition` (everything comes back to the center).
...
| # | Action | Expected |
|---|---|---|
| 1 | `/gideon` out of instance | "Your partner: …" + list of pairs (`assignment` block prepared by GIDEON); with no plan: `No out-of-game plan loaded (optional).` and **no** mention of a Discord command |
| 2 | `/gideon plan` | the detailed plan in the chat (role, position, meeting) |
| 3 | `bindings`: Options > Keybindings > GideonRaid, assign a key | the binding shows up; the key opens/closes the panel |
| 3b | **Before the pull**: `/gideon` → **PLACE INTERMISSION PANEL** (or `/gideon inter place`) | the intermission panel appears, can be **dragged** where the player wants it; the body recalls the ping keybind and the lead **and says exactly what to do** (fifth in-game feedback): *place the panel where you want it to appear, then press OK: during the fight it opens by itself*; the **OK** button is on screen (right of the frame, next to **Close**), **saves the position and closes** the panel. **Close** and the **X** cancel instead. Re-doable, and usable several times |
| 3c | Bind each ping: Options > Keybindings > **ping system** (or Raccourcis) | one key per ping exists (Avertissement / En route / Aide) — **already measured**; `/gideon inter ping` then shows `PING: Avertissement - press <key>` if the binding command name matches a candidate (TBD item 1 of `docs/INTERMISSION-COACH.md` §9) |
| 4 | **Name the target first**: `/gideon boss <id>` with the id captured per §3.5e (until then, nothing opens by itself). Then enter *Entombed Sentinels* and pull the boss | `ENCOUNTER_START` **arms the schedule** (the panel does **not** open at the pull) and the chat confirms the number of planned intermissions and the 2 s lead. Only the configured target arms anything; the four arguments are read **once, under `pcall`**, and only to compare the encounter id / name (§3.5e) |
...
| 11 | `/gideon lang` on a frFR client and on an enUS client | detected language correct, text in the right language, and the **ping label follows it** (`PING : Avertissement` in French); TBD item 6 of `docs/INTERMISSION-COACH.md` §9 |
| 12 | `/gideon ping` then `/gideon ping anchors|color|none`, then re-open the intermission panel | the header banner switches between **PING: YES** and **PING: NO** and the action line follows the policy (never a contradictory order for the same role) — the policy itself stays **out** of the panel |
...
| 14c | Move the **main panel** (or ask the other players to move theirs) during the raid, then `/reload` between two intermissions | the panel is draggable (it was frozen before), the position is kept after the `/reload`, and `/gideon lock` keeps it in place if it hides something |
...
| # | Action | Expected |
|---|---|---|
| 1 | `/gideon`, then hover the **X** in the top right corner | a short tooltip reads `Close` (English) / `Fermer` (French on a frFR client); the X itself is the label from `Core/Locale.lua` |
| 2 | Click that **X** | the main panel closes; `/gideon` brings it back (the plan and the buttons are unchanged) |
| 3 | `/gideon inter place`, then click the **X** of the intermission panel | the placement is **cancelled** exactly like the Close button: the panel closes and **no** position is saved; `/gideon inter place` works again |
| 4 | `/gideon inter start` (or wait for a real intermission), then click the **X** | the panel hides; the intermission clock keeps running, the panel **closes by itself** at the end and **opens again at the next intermission** (nothing was disarmed) |
| 5 | `/gideon sim inter` (same as the **SIMULATION** button of the main panel) | the chat states `SIMULATION (no boss, no raid): the intermission panel opens RIGHT AWAY. Click your composition, then close it yourself (X or Close button). The ENCOUNTER_START timeline is NOT armed.`; the panel opens **IMMEDIATELY** (no delay) with the **two-line** banner `SIMULATION - NO BOSS, NO RAID` / `SINGLE REHEARSAL - YOU CLOSE THE PANEL YOURSELF` **and the three composition buttons** |
...
| 12 | Start a rehearsal, then close it with `/gideon sim stop` (or the **Close** button, or the **X**, or the close button of the ping help window) | it closes at once with an honest report (`Simulation closed (no boss, no raid, nothing was published).`); `/gideon sim stop` again answers `No simulation running.` |
| 13 | Start a simulation, then **pull a boss** (or `/gideon inter start`) | the simulation is **stopped at once** by `ENCOUNTER_START` and the normal schedule is armed **exactly once** (no double state, no panel left open) |
| 14 | `/gideon sim`, then `/gideon sim bidon`, then `/gideon sim inter cycles=3` | the help lists the three commands and `/gideon pinghelp`; an unknown sub-command **and a trailing option** are **refused** with a message (never guessed, and no panel opens) |
| 15 | Drag the **main panel** (`/gideon`), then `/reload`, then `/gideon` again | the panel moves with the left button (it was **frozen** before: `lockPanel` hard-coded to `true`), the position survives the `/reload` and the panel reopens **where it was left** |
| 16 | `/gideon lock`, then try to drag it; `/gideon unlock`, then drag it again | locked: the panel does not move and the chat explains it (the LOCK PANEL button shows UNLOCK PANEL); unlocked: the drag works again and the choice survives a `/reload` |
| 17 | Drag the **ping help window** (`/gideon sim ping`), close it, open it again | the window is draggable and **reopens where it was dragged** (position persisted like the main panel) |
| 18 | `/gideon resetposition` | the main panel, the intermission panel and the ping help window all come back to the **center** of the screen |
| 19 | Open the main panel in French, then in English (`/gideon lang fr` / `/gideon lang en`) | **no label touches or leaves the frame** in either language and the four buttons keep the order **PLACE LE PANNEAU -> SIMULATION: TE PINGER -> SIMULATION: GROUPE INTER -> VERROUILLER** (the utility separated by a small space, all the same width) |
...
| # | Action | Expected |
|---|---|---|
| 1 | `/gideon inter place` | the panel shows the placement text **including** *Place the panel where you want it to appear, then press OK: during the fight it opens by itself*; the **OK** button is **on screen**, at the bottom right next to **Close** (not clipped, not overlapping it) |
| 2 | Drag the panel where you want it, then click **OK** | the panel closes, the chat confirms `Placement saved. Pull when you want: the panel opens by itself before each intermission.`; `/gideon inter place` shows the panel **at the new position** |
| 3 | `/gideon inter place`, then click the **X** (or **Close**) | the placement is **cancelled**: nothing is saved (that is the difference with OK) |
| 4 | `/gideon sim inter` | the panel opens right away with the banner **and the three composition buttons** (`1 vert + 3 rouges` / `2 verts + 2 rouges` / `3 verts + 1 rouge`, French labels in game), each with its **three lines of label fully inside its button** — no character out of the frame, no label touching the border |
...
| # | Action | Expected |
|---|---|---|
| 1 | `/gideon sound` | the chat answers `Assignment sound: enabled - one soundboard per composition (1V3R / 2V2R / 3V1R), played once when you click your composition. /gideon sound on\|off to change it, /gideon sound test 1v3r\|2v2r\|3v1r to hear one now.` (`actif`/`desactive` in French) |
| 2 | `/gideon sound test 1v3r`, then `2v2r`, then `3v1r` | each command **names its own file** in the chat: `Sound test: 1V3R (assign-1v3r.ogg)…`, `Sound test: 2V2R (assign-2v2r.ogg)…`, `Sound test: 3V1R (assign-3v1r.ogg)…` — with the **placeholders** you hear nothing (the files are silence, by design) but the plumbing is proven; **once the raid lead's recordings are in place**, each command plays its own soundboard: `1v3r` must play the **1V3R** sound, not the `3v1r` one — hear the three in a row to tell them apart |
| 3 | `/gideon sound test bidon` | refused: `Unknown sound 'bidon': accepted values are 1v3r, 2v2r, 3v1r, start.` and **nothing** is played |
| 4 | `/gideon sound yes` (or any value that is not `on`/`off`) | refused: `Unknown value 'yes': accepted values are on, off.`; `/gideon sound` still reports the **previous** state (nothing was persisted) |
| 5 | `/gideon sim inter` (or a real intermission), then click a composition | the soundboard of **that** state plays **exactly once**, **at the click** — the state, the role, `PING: YES/NO` and the action line appear as usual, in the same instant |
| 6 | Click the same composition again (`/gideon inter 3V1R` twice, or force a click) | **no second sound**: one playback per assignment |
...
| 8b | **The intermission START sound**: `/gideon sound test start` | the chat answers `Intermission start sound: intermission-start.ogg should have played (once at the beginning of every intermission)…` and the raid lead's recording is heard (**this file is NOT a placeholder**) |
...
| 8d | `/gideon sim inter` (then a second `/gideon sim inter` after closing it) | the start sound plays **once per rehearsal**, like a real intermission |
| 8e | `/gideon sound off`, then `/gideon sim inter` | **nothing plays** (neither the start sound nor a soundboard) and the panel is unchanged; `/gideon sound on` restores both |
| 9 | `/gideon sound off`, then click a composition | **nothing plays**, and the panel is **unchanged**: state, role, `PING` banner and action line are all there (no Lua error, no frozen panel). `/gideon inter status` reports `assignment sound: disabled` |
| 10 | `/gideon sound on`, then `/gideon sound test 3v1r` | the sound plays again |
| 11 | Switch the language (`/gideon lang fr`, then `/gideon lang en`) and redo points 1, 2 and 4 | the same messages in French (`/gideon sound` = état, `/gideon sound test` = « Test du son : … »), and an unknown value is still refused: the messages follow the active language like the rest of the addon |
| 12 | `/reload`, then `/gideon sound` | the preference survived (a `/gideon sound off` before the reload is still off) — it lives in `GideonRaidDB.intermission.soundEnabled` |
...
| # | Action | Expected |
|---|---|---|
| 1 | Fresh install (or a client whose SavedVariables were never touched): `/reload` and watch the chat | **no warning**: a target is delivered, so the panel will open by itself. `/gideon boss` reports `Auto-open target boss: ids 3445, names entombed sentinels, sentinelles inhumées` **and** the two lines that say where it comes from (`Target source: the default DELIVERED with the addon` + `Default delivered with the addon (it needs NO command and NO SavedVariables): id 3445, names Entombed Sentinels, Sentinelles inhumées.`) |
| 2 | `/gideon boss list` | each id is labelled with its provenance: `Auto-open target ids: 3445 (addon default)` |
...
| 6 | `/gideon boss clear`, then pull **Entombed Sentinels** | **nothing opens** and the chat says the target was cleared **on purpose**: `Auto-open target is cleared on purpose (/gideon boss clear) … /gideon boss 3445 puts the target of the addon back` — the delivered default must **not** come back by itself |
| 7 | `/gideon boss 3445` (the id typed back) | `Target encounter id 3445 added…` and the panel opens on the next pull; `/gideon boss list` now reads `3445 (added by you)` with `Target source: ONLY the entries added by a player` |
| 8 | `/gideon boss 3445` a **second time** | idempotent: no duplicate in `/gideon boss list`, the target does not change |

**B. The filter in a raid evening**

| # | Action | Expected |
|---|---|---|
| 9 | Pull **a boss that is not the target**, with the delivered target in place | the panel **never opens** (no ticker, no schedule armed) and the chat prints the refusal, which names `/gideon boss <id>` — an explanation, not a bug report |
| 10 | `/gideon boss abc`, `/gideon boss 0`, `/gideon boss -3`, `/gideon boss 12.5` | each is refused (`Unknown encounter id '…': a POSITIVE INTEGER is expected …`) and `/gideon boss` still shows the **previous** target: nothing was written |
| 11 | `/gideon idlog peut-etre` | refused (`Unknown value 'peut-etre': accepted values are on, off.`) and `/gideon idlog` still reports the **previous** state |
| 12 | `/gideon lang fr`, then `/gideon boss` and `/gideon boss list` | the same messages in French (`Boss cible de l'ouverture auto : …`, `Origine de la cible : le defaut LIVRE avec l'addon …`): every new string follows the active language like the rest of the addon |
| 13 | `/gideon inter on`, then pull **any** boss | the panel opens on that encounter (`Encounter started: the MANUAL OVERRIDE (/gideon inter on) opens the panel for it, whatever the boss…`), and at the end of that encounter the chat says the override is **consumed**: the next boss does **not** open it |
...
| 15 | `/gideon lang fr`, then `/gideon boss` and `/gideon idlog` | the same messages in French (`Boss cible de l'ouverture auto : …`, `Journal des ids d'encounter : …`) |

**C. Measuring the id of ANOTHER boss (the procedure that produced 3445)**

| # | Action | Expected |
|---|---|---|
| 16 | `/gideon idlog on`, then pull the boss | one line per `ENCOUNTER_START`: `encounter seen: id=<ID> name=<NAME> difficulty=<d> group=<n>` — **write down the `id=` value**; a value the client hides prints `unreadable`, never a fake number |
| 17 | `/gideon boss list` | `Encounters memorized by the idlog (1, newest first):` then the line indented: the id is readable **after** the fight, no screenshot needed |
| 18 | `/gideon boss <THE_ID>` | `Target encounter id <ID> added…` and that boss opens the panel **too** (it is added to the delivered one, it never replaces it). `/gideon idlog off` when finished |
| 19 | `/gideon boss name <the exact name your client displays>` | the name is added (secondary criterion) and `/gideon boss list` shows it labelled `added by you`. **Language-dependent: paste only what the idlog printed on YOUR client; no translation is ever written for you** |

**Failure reading**: if the panel opens on a boss that is not the target, look at
`/gideon boss list` — either that id is in the list (remove it with `/gideon boss clear`
then re-add the right ones) or `overrideEncounter` is armed (`/gideon inter off` then
`/gideon inter on` clears it). If `/gideon boss <id>` is refused, the value is not a
...
delivered target, check `/gideon boss`: if it reads `cleared ON PURPOSE`, a
`/gideon boss clear` was played (the SavedVariables keep `bossTargetCleared = true`) —
that is the intended behaviour, and `/gideon boss 3445` restores it.

### 3.5f In-game protocol — `/gideon diag`, the health report (5 min, ALONE)
...
| # | Action | Expected |
|---|---|---|
| 1 | `/gideon diag` with the game sound **ON** (the normal raid case) | a read-only report: the target (`auto-open target: ids 3445 …`), its source, the delivered default, `encounter id log: …`, `intermission ping policy: anchors - …`, and the four sound files listed as **`NOT TESTED`** — and **absolutely no sound is played** (this is the promise: no noise in a raid). The report ends with the two ways to get a verdict |
| 2 | `/console Sound_MasterVolume 0` (master volume 0, channel still enabled), then `/gideon diag` | the four files are now probed for real: four **`present and playable [OK]`**, with `the check DID run for real … NOTHING was audible`. This is the **confirmation that a volume of 0 does not make the client answer "nothing will play"**; if instead **all four** read `NOT playable [KO]`, the report's own caveat fires (`ALL FOUR files came back as not playable`) and the gate has to be revisited (`Core/Diag.probeGate` is the single place). Then `/console Sound_MasterVolume 1` |
| 3 | Ctrl+S (or Options > Sound) to **disable the sound**, then `/gideon diag` | four `NOT TESTED` + `the sound is OFF (or unreadable) in this client, and a DISABLED channel answers 'nothing will play' even for a file that is really there - the verdict would be a lie`: **nothing is played**. Turn the sound back on |
| 4 | **Negative case**: rename `Sound/assign-3v1r.ogg` to `assign-3v1r.ogg.bak` (or replace its content with a non-audio file), **restart the client** (a `/reload` is NOT enough for sound files), set the master volume to 0, `/gideon diag` | that line alone reads `Sound/assign-3v1r.ogg: NOT playable [KO]` while the other three read `[OK]`. Restore the file and restart again. This is the only way to validate a real KO |
| 5 | `/gideon diag` twice in a row | idempotent and read-only: nothing is written in `GideonRaidDB` (`/gideon boss`, `/gideon boss list` and `/gideon idlog` are unchanged afterwards), the panel does not open, and the four sounds are not played twice |
| 6 | `/gideon lang fr`, then `/gideon diag` | the whole report in French (`--- DIAGNOSTIC (lecture seule …)`, `cible de l'ouverture auto : …`, `NON jouable [KO]`): every line goes through `Core/Locale.lua` like the rest of the addon |
...
turn it on; CVar unreadable → the client refused the read, `/gideon sound test
...
| Wrong language served (English/French) | 1c | 21 tests (`resolve`, `t`/`format` fallbacks, `GetLocale` stubbed `frFR`/`enUS`/`deDE`, `/gideon lang`, missing key) |
...
| Ping keybind not found (wrong candidate name) | 1b + 3 | the panel shows **no key** and asks for a keybind in Options > Keybindings; `/gideon inter ping` prints the candidates tried (TBD item 1 of §9) |
...
| Unknown ping policy silently accepted | 1b + 2 | pure resolver (unknown → `anchors`), `/gideon ping` refuses an unknown value and writes nothing |
...
| **Sound still playing when the player muted it** | 1f + 2 + 3 | total resolver (only an exact `false` mutes), `/gideon sound off` then a click asserts **zero** `PlaySoundFile` call while the panel still renders; protocol §3.5d points 9/10 |
...
| **A deliberate `/gideon boss clear` is silently undone by the delivered default** (or the reverse: a lost configuration mistaken for a choice) | 1g + 2 | `bossTargetCleared` (an **exact `true`** only) is distinct from an empty list: after a clear the delivered default is **dropped** (`bossTargetSource = "cleared"`, nothing opens) until `/gideon boss <id>` adds one; `/gideon boss`, `/gideon boss list` and `/gideon diag` all say **where** the target comes from, and `/gideon boss list` labels every entry *addon default* / *added by you*; protocol §3.5e A rows 6–8 |
| **`/gideon diag` makes a noise in a raid** (an in-game health check must never play a sound) | 1h + 2 + 3 | the audio probe is behind the **silence gate** `Core/Diag.probeGate`: it runs only with the Master channel **enabled** (otherwise a disabled channel answers "nothing will play" even for a present file — a lie) **and** its volume at **0** (so the playback is inaudible). `diag_spec.lua` asserts that `/gideon diag` calls `PlaySoundFile` **zero** times when the sound is on or off, and exactly 4 times (channel `Master`, the right paths) when the volume is 0; `guard_spec.lua` asserts the gate is what the rendering layer obeys; protocol §3.5f rows 1–3 |
...
| **The target boss id is invented / wrong** (the id used to be unknown) | 1g + 2 + 3 | nothing is guessed: the delivered id **3445** is the one **MEASURED IN GAME** (raid lead, 2026-09-24, `/gideon idlog on`, heroic 20-player pull: `encounter seen: id=3445 name=Sentinelles inhumées difficulty=15 group=20`), `/gideon boss` refuses anything that is not a **positive integer without persisting it**, and the id of any other boss comes only from **`/gideon idlog on` in game** (§3.5e C: the chat prints `id=…` and `/gideon boss list` keeps the last 10 encounters) |
